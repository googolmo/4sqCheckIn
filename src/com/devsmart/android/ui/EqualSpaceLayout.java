...
public class EqualSpaceLayout extends ViewGroup {
	...
	public EqualSpaceLayout(Context context, AttributeSet attrs) {
...

		{
			...
			int layoutwidth = a.getLayoutDimension(0,
					ViewGroup.LayoutParams.WRAP_CONTENT);
			int layoutheight = a.getLayoutDimension(1,
					ViewGroup.LayoutParams.WRAP_CONTENT);
			setLayoutParams(new ViewGroup.LayoutParams(layoutwidth,
					layoutheight));
			a.recycle();
		}
		...
		{
			TypedArray a = context.obtainStyledAttributes(attrs,
					new int[] { android.R.attr.orientation });
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		int maxChildWidth = mOrientation == LinearLayout.HORIZONTAL ? (MeasureSpec
				.getSize(widthMeasureSpec) / mNumVisibleChildren) : MeasureSpec
				.getSize(widthMeasureSpec);
		int maxChildHeight = mOrientation == LinearLayout.VERTICAL ? (MeasureSpec
				.getSize(heightMeasureSpec) / mNumVisibleChildren)
